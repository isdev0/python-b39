...
def test_info_on_home_page(app, index=0, scope=None):
...
def test_random_contact_info_on_home_page(app):
    if app.contact.count() == 0:
        app.contact.create(Contact(firstname="FN TestForInfoDisplaying", lastname="LN TestForInfoDisplaying", mobile="(111) 111-11-111", email2="email2"))

    index = randrange(app.contact.count())
    print("\nRandom Index: " + str(index))

    test_info_on_home_page(app, index)
